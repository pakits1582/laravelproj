@extends('layout')
@section('title') {{ 'Update Fee' }} @endsection
@section('content')
    <!-- Begin Page Content -->
            ...
            <div class="row">
                <div class="col-lg-7 mx-auto">
                    <div class="bg-white rounded-lg shadow-sm p-5">
                        <h1 class="h3 mb-0 text-primary font-weight-bold">Update Fee</h1>
                        ...
                        <div id="nav-tab-card" class="tab-pane fade show active">
                            ...
                            @endif
                            <form method="POST" action="{{ route('fees.update', ['fee' => $fee->id]) }}"  role="form">
                            @csrf
                            @method('PUT')
                                    ...
                                    @if ($fee_types)
                                        @foreach ($fee_types as $fee_type)
                                            <option 
                                                ...
                                                {{ ($fee->fee_type_id == $fee_type->id) ? 'selected' : '' }}
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="code" placeholder="" class="form-control text-uppercase" value="{{ (old('code')) ? old('code') : $fee->code }}">
                                @error('code')
                                    <p class="text-danger text-xs mt-1">{{$message}}</p>
                                @enderror
                            </div>
                            <div class="form-group">
                                <div class="form-check form-check-solid">
                                    <input class="form-check-input" id="iscompound" type="checkbox" value="1" name="iscompound" {{ (old('iscompound') === 1) ? 'checked' : (($fee->iscompound === 1) ? 'checked' : '') }} >
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="default_value" placeholder="0.00" class="form-control text-uppercase" value="{{ (old('default_value')) ? old('default_value') : $fee->default_value }}">
                                @error('default_value')
                                    <p class="text-danger text-xs mt-1">{{$message}}</p>
                                @enderror
                            </div>
                            <button type="submit" class="subscribe btn btn-primary btn-block rounded-pill shadow-sm"> Update Fee  </button>
                            </form>
                        </div>
                        <!-- End -->
